...
interface OrbProps {
  /** Set once the Intro overlay has finished, to trigger a Waypoint re-measure. */
  introDone: boolean
}
...
export function Orb({ introDone }: OrbProps) {
  ...
  useEffect(() => {
    ...
    let prevPx = px
    let prevPy = py
    const ghosts: Point[] = Array.from({ length: GHOST_COUNT }, () => ({ x: px, y: py }))

    let rafId: number | null = null
    const loop = () => {
      const scrollY = window.scrollY
      const vh = window.innerHeight
      const i = orbSegment(scrollY, ats)
      const defA = resolved[i]
      const defB = resolved[i + 1]
      ...
      if (!defB) {
        ...
        const zone = Math.min(vh * ZONE_VH_RATIO, Math.max(1, ats[i + 1] - ats[i]))
        const u = (scrollY - (ats[i + 1] - zone)) / zone
      ...
      px += (tx - px) * FOLLOW_SMOOTHING
      py += (ty - py) * FOLLOW_SMOOTHING
      const vx = px - prevPx
      const vy = py - prevPy
      prevPx = px
      prevPy = py
      ...
      wrap.style.transform = `translate(${px - size / 2}px,${py - size / 2}px) rotate(${angle}deg) scale(${(1 + stretch) * idle},${(1 - stretch * 0.65) * idle})`
      ...
      if (periodDef.end) {
        const periodPos = posOf(periodDef)
        const near = Boolean(defA.end) && Math.hypot(px - periodPos.x, py - periodPos.y) < PERIOD_NEAR_PX
        periodDef.el.style.opacity = near ? '0' : '1'
      }
